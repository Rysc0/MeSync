...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchListsData() {
    await fetch('http://127.0.0.1:8123/getFilteredListsOnBoard') // Replace with your API URL
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  async function populateBoardsDropdown(data) {
  ...
  async function populateListsDropdown(data) {
    // generate options based on the response
    for (i = 0, len = data.length; i < len; i++) {
      const option = document.createElement('option');
      option.value = data[i].id; // Adjust based on API response structure
